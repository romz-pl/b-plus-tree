...
#include "btree_default_map_traits.h"
#include "node.h"
#include "btree_inner_node.h"
#include "btree_leaf_node.h"
#include "btree_iterator.h"
#include "btree_pair_to_value.h"
#include "btree_const_iterator.h"
#include "btree_reverse_iterator.h"
#include "btree_const_reverse_iterator.h"
#include "btree_tree_stats.h"
...
public:
    ...
    static const size_type innerslotmax = traits::innerslots;
    ...
    static const size_type mininnerslots = (innerslotmax / 2);
...
private:
    ...
    typedef btree_inner_node< _Key, _Traits, _Alloc > inner_node;
...
public:
    ...
    typedef btree_iterator<_Key, _Data, _Value, _Compare, _Traits, _Duplicates, _Alloc, _UsedAsSet>                 iterator;
    typedef btree_const_iterator<_Key, _Data, _Value, _Compare, _Traits, _Duplicates, _Alloc, _UsedAsSet>           const_iterator;
    typedef btree_reverse_iterator<_Key, _Data, _Value, _Compare, _Traits, _Duplicates, _Alloc, _UsedAsSet>         reverse_iterator;
    typedef btree_const_reverse_iterator<_Key, _Data, _Value, _Compare, _Traits, _Duplicates, _Alloc, _UsedAsSet>   const_reverse_iterator;
...
public:
    ...
    typedef btree_tree_stats<_Traits> tree_stats;
...
private:
    ...
    inline inner_node * allocate_inner(size_type level)
    {
...
private:
    ...
    void clear_recursive(node* n)
    {
        if (n->isleafnode())
        {
            ...
            for (size_type slot = 0; slot < leafnode->slotuse; ++slot)
            {
        ...
        else
        {
            ...
            for (size_type slot = 0; slot < innernode->slotuse + 1; ++slot)
            {
...
private:
    ...
    inline size_type find_lower(const node_type* n, const key_type& key) const
    {
        if (0 && sizeof(n->slotkey) > traits::binsearch_threshold)
        {
            if (n->slotuse == 0) return 0;

            size_type lo = 0, hi = n->slotuse;
    ...
    template <typename node_type>
    inline size_type find_upper(const node_type* n, const key_type& key) const
    {
        if (0 && sizeof(n->slotkey) > traits::binsearch_threshold)
        {
            if (n->slotuse == 0) return 0;
...
public:
    ...
    bool exists(const key_type& key) const
    {
        ...
        while (!n->isleafnode())
        {
            ...
            size_type slot = find_lower(inner, key);
        ...
        size_type slot = find_lower(leaf, key);
    ...
    iterator upper_bound(const key_type& key)
    {
        ...
        {
            const inner_node* inner = static_cast<const inner_node*>(n);
            size_type slot = find_upper(inner, key);
        ...
        size_type slot = find_upper(leaf, key);
        return iterator(leaf, slot);
    }
...
private:
    ...
    struct node * copy_recursive(const node* n)
    {
        ...
        else
        {
            ...
            for (size_type slot = 0; slot <= inner->slotuse; ++slot)
            {
...
private:
    ...
    */
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, size_type addslot)
    {
        BTREE_ASSERT(inner->isfull());

        size_type mid = (inner->slotuse >> 1);
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        for (size_t i = 0; i < num_leaves; ++i)
        {
            ...
            leaf->slotuse = static_cast<size_type>(num_items / (num_leaves - i));
        ...
        for (size_t i = 0; i < num_parents; ++i)
        {
            ...
            n->slotuse = static_cast<size_type>(num_leaves / (num_parents - i));
            ...
            for (size_type s = 0; s < n->slotuse; ++s)
            {
        ...
        for (size_type level = 2; num_parents != 1; ++level)
        {
            ...
            for (size_t i = 0; i < num_parents; ++i)
            {
                ...
                n->slotuse = static_cast<size_type>(num_children / (num_parents - i));
                BTREE_ASSERT(n->slotuse > 0);
                --n->slotuse; // this counts keys, but an inner node has keys+1 children.
...
private:
    ...
    result_t erase_one_descend(const key_type& key,
                               ...
                               inner_node* parent, size_type parentslot)
    {
        if (curr->isleafnode())
        {
                                ...
                                inner_node* parent, size_type parentslot)
    {
        ...
        else // !curr->isleafnode()
        {
            ...
            result_t result;
            size_type slot = find_lower(inner, iter.key());
    ...
    /// removed by the calling parent node.
    static result_t merge_inner(inner_node* left, inner_node* right, inner_node* parent, size_type parentslot)
    {
    ...
    /// if possible.
    static result_t shift_left_leaf(leaf_node* left, leaf_node* right, inner_node* parent, size_type parentslot)
    {
        BTREE_ASSERT(left->isleafnode() && right->isleafnode());
        BTREE_ASSERT(parent->level == 1);
        ...
        size_type shiftnum = (right->slotuse - left->slotuse) >> 1;
    ...
    /// updated if possible.
    static void shift_left_inner(inner_node* left, inner_node* right, inner_node* parent, size_type parentslot)
    {
        BTREE_ASSERT(left->level == right->level);
        BTREE_ASSERT(parent->level == left->level + 1);
    ...
    /// if possible.
    static void shift_right_leaf(leaf_node* left, leaf_node* right, inner_node* parent, size_type parentslot)
    {
        BTREE_ASSERT(left->isleafnode() && right->isleafnode());
        BTREE_ASSERT(parent->level == 1);
        ...
        size_type shiftnum = (left->slotuse - right->slotuse) >> 1;
    ...
    /// right so that both nodes are equally filled. The parent node is updated
    /// if possible.
    static void shift_right_inner(inner_node* left, inner_node* right, inner_node* parent, size_type parentslot)
    {
        BTREE_ASSERT(left->level == right->level);
        BTREE_ASSERT(parent->level == left->level + 1);
...
private:
    ...
    static void print_node(std::ostream& os, const node* node, size_type depth = 0, bool recursive = false)
    {
        ...
        else
        {
            ...
            for (size_type slot = 0; slot < innernode->slotuse; ++slot)
            {
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats& vstats) const
    {
        ...
        if (n->isleafnode())
        {
            ...
            for (size_type slot = 0; slot < leaf->slotuse - 1; ++slot)
            {
        ...
        else // !n->isleafnode()
        {
            ...
            for (size_type slot = 0; slot < inner->slotuse - 1; ++slot)
            {
    ...
    void verify_leaflinks() const
    {
        ...
        while (n)
        {
            ...
            for (size_type slot = 0; slot < n->slotuse - 1; ++slot)
            {
...
private:
    ...
    struct dump_header
    {
        ...
        /// sizeof(key_type)
        size_type key_type_size;

        /// sizeof(data_type)
        size_type data_type_size;

        /// Number of slots in the leaves
        size_type leafslots;

        /// Number of slots in the inner nodes
        size_type innerslots;
    ...
    node * restore_node(std::istream& is)
    {
        ...
        else
        {
            ...
            *newinner = inner;
            ...
            for (size_type slot = 0; slot <= newinner->slotuse; ++slot)
            {
