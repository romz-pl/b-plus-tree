...
public:
    ...
    void erase( iterator begin, iterator end )
    {
        ...
        typename CheckerType::iterator checker_end = end == tree_.end() ? checker_.end() : checker_.find( end.key() );
        if( end != tree_.end() )
        {
...
//
const std::vector< int >& GenerateNumbers( size_t n, size_t maxval )
{
...
void DoTest( const char *name, T *b, const std::vector< V > &values )
{
    ...
    // Test dumping of the btree to an ostream. There should be 1 line for each value.
